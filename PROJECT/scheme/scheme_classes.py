...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        "*** YOUR CODE HERE ***"
        currEnv = self
        while currEnv is not None:
            if symbol in currEnv.bindings:
                return currEnv.bindings[symbol]
    ...
    def make_child_frame(self, formals, vals):
        ...
        key = formals
        val = vals
        while key is not nil:
            frame.define(key.first, val.first)
            key, val = key.rest, val.rest
