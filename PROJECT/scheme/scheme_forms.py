...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # procedure = LambdaProcedure(formal, body, env)
        # env.bindings[signature.first] = procedure

        # resolve 2
        formal = signature.rest
        body = expressions.rest
        procedure = do_lambda_form(Pair(formal, body), env)
...
def do_and_form(expressions, env):
    ...
    subExpr = expressions
    while subExpr.rest is not nil:
        val = scheme_eval(subExpr.first, env)
        if is_scheme_false(val):
            return val
        subExpr = subExpr.rest

    return scheme_eval(subExpr.first, env, True)
    # END PROBLEM 12
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                return test
            ret = clause.rest
            while ret.rest is not nil:
                scheme_eval(ret.first, env)
                ret = ret.rest

            return scheme_eval(ret.first, env, True)
            # END PROBLEM 13
