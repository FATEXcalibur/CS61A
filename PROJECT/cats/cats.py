...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(paragraph):
        a_list = remove_punctuation(paragraph).split(' ')
        for word in a_list:
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    index = 0
    while index < len(typed) and index < len(source):
        if typed[index] != source[index]:
            break
        index += 1

    radio = index / len(source)
...
def fastest_words(words_and_times):
    ...
    for j in word_indices:
        index = min(player_indices, key = lambda p: times[p][j])
        res[index] += [words[j]]
