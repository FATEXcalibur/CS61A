...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = 0
        while i < index and not currPlace.is_hive:
        ...
        while not currPlace.bees and not currPlace.is_hive and index >= self.lower_bound and index < self.upper_bound:
            currPlace = currPlace.entrance
            index += 1

        if currPlace.is_hive:
            return None
        if currPlace.bees:
            return random_bee(currPlace.bees)
...
class HungryAnt(Ant):
    """
    Select a random Bee from its place and deal damage to the Bee equal to the Bee's health
    """
...
class ScubaThrower(ThrowerAnt):
    """
    is a more costly and is_waterproof
    """
    name = 'Scuba'
    food_cost = 6
    is_waterproof = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not hasattr(target, 'slow_applied'):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        currPlace = self.place 
        d = 0
        while not currPlace.is_hive:
            if currPlace.ant and currPlace.ant is not self:
                res[currPlace.ant] = d
                if currPlace.ant.is_container and currPlace.ant.ant_contained and currPlace.ant.ant_contained is not self:
                    res[currPlace.ant.ant_contained] = d
            
            for b in currPlace.bees:
                res[b] = d
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        res = self.damage - distance * 0.25 - self.insects_shot * 1 / 16
        if res < 0:
            return 0
        return res
        # END Problem EC 4
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if hasattr(self, 'scared') and self.scaredTurns > 0:
