...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def func(paragraph):
        temp = [lower(x) for x in split(remove_punctuation(paragraph))]
        for word in temp:
            if word in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    i = 0
    while i < total_words and i < total_source_words:
        if typed_words[i] == source_words[i]:
            accurate_words += 1
        i += 1
    return (accurate_words / total_words) * 100
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    wordsNum = len(typed) / 5
    return wordsNum / (elapsed / 60)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if temp <= limit and temp < minDiff:
            minDiff = temp
            ret = word
...
def fastest_words(words_and_times):
    ...
    for x in player_indices:
        ret.append([])
    for iword in word_indices:
        temp_time = float('inf')
        temp_player = 0
        for iplayer in player_indices:
            # todo: find the min time of player
            iplayer_time = get_time(times, iplayer, iword)
            if iplayer_time < temp_time:
                temp_time = iplayer_time
                temp_player = iplayer
        ret[temp_player].append(words[iword])
