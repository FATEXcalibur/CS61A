...
def balanced(m):
    ...
    leftArm = left(m)
    rightArm = right(m)
    leftMobile = end(leftArm)
    rightMobile = end(rightArm)

    if total_mass(leftMobile) * length(leftArm) != total_mass(rightMobile) * length(rightArm):
        return False

    return balanced(leftMobile) and balanced(rightMobile)
...
def max_path_sum(t):
    ...
    maxVal = []
    for branch in branches(t):
        maxVal += [max_path_sum(branch)]

    return label(t) + max(maxVal)
